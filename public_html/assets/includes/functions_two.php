...
function Wo_PageData($page_id = 0) {
    ...
    if (!empty($wo['page_categories'][$fetched_data['page_category']])) {
        ...
        $fetched_data['category'] = Wo_GetInterest($fetched_data['page_category']);
    }
...
function Wo_ContactSendMessage($data = array()) {
    global $wo, $sqlConnect, $mail;
    $email_from      = $data['from_email'] = Wo_Secure($data['from_email']);
    $to_email        = $data['to_email'] = Wo_Secure($data['to_email']);
    $email=  $data['reply-to'] = Wo_Secure($data['reply-to']);
    $subject         = $data['subject'];
    $message_body    = mysqli_real_escape_string($sqlConnect, $data['message_body']);
    $data['charSet'] = Wo_Secure($data['charSet']);
    $user_id   = Wo_Secure($wo['user']['user_id']);
            $query_one = mysqli_query($sqlConnect, "INSERT INTO " . T_EMAILS . " (`user_id`,`email_to`,`subject`, `message`) VALUES ('{$user_id}','{$email}', '{$subject}', '{$message_body}')");
            if ($query_one) {
                //mail($to_email,$subject,$message_body,$email_from);
                return true;
            }
}
    ...
    $user_id   = Wo_Secure($wo['user']['user_id']);
       ...
       if (isset($data['insert_database'])) {
        ...
        if ($data['insert_database'] == 1) {
            $user_id   = Wo_Secure($wo['user']['user_id']);
            $query_one = mysqli_query($sqlConnect, "INSERT INTO " . T_EMAILS . " (`user_id`,`email_to`,`subject`, `message`) VALUES ('{$user_id}','{$to_email}', '{$subject}', '{$message_body}')");
            if ($query_one) {
                return true;
            }
        }
        return $data['insert_database'];
        exit();
    }
    if ($wo['config']['smtp_or_mail'] == 'mail') {
        $mail->IsMail();

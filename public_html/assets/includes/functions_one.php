...
function Wo_UserIdFromPhone($phone) {
    global $sqlConnect;
    if (empty($phone)) {
        return false;
    }
...
function Wo_UpdateUser($registration_data, $invited = false) {
     global $wo, $sqlConnect;
     echo "hi";
    ...
    $query                               = mysqli_query($sqlConnect, "Update" . T_USERS . " set ({$fields})= ({$data}) where phone_number='8054558410'");
    return true;
}
...
function Wo_GetAllUsers($limit = '', $type = 'user', $filter = array(), $after = '') {
    global $wo, $sqlConnect;
    $data      = array();
...
function Wo_RegisterFollow($following_id = 0, $follower_id = 0) {
    ...
    // message code start
        $query = mysqli_query($sqlConnect, "SELECT COUNT(*) as row_count FROM wo_followers WHERE `following_id`='$following_id' and `date`='$date'");
        $fetched_data = mysqli_fetch_assoc($query);
       // echo $fetched_data['row_count'];
  
        if($fetched_data['row_count'] == 0){
             $messages=$follower_data['first_name']." wants to be friends with you on mitrah.in First Indian Social Networking Website. Please login to approve.";
            $m=$following_data['phone_number'];
    ...
    $query = mysqli_query($sqlConnect, " INSERT INTO " . T_FOLLOWERS . " (`following_id`,`follower_id`,`active`,`date`) VALUES ({$following_id},{$follower_id},'{$active}','{$date}')");
    if ($query) {
        if ($active == 1) {
...
function Wo_RegisterNotification($data = array()) {
    ...
    // start
     $date = date('Y-m-d');
        $query_one = "SELECT `id` FROM " . T_NOTIFICATION . " WHERE `recipient_id` = " . $recipient['user_id'] . "  AND `date`='$date' " ;
        
            $sql_query_one = mysqli_query($sqlConnect, $query_one);
        
                if(mysqli_num_rows($sql_query_one) == 0){
                      $follower_data  = Wo_UserData($data['notifier_id']);
                      $following_data = Wo_UserData($recipient['user_id']);
                      $lastseen = Wo_Secure($following_data['lastseen']);
                      $time     = time() - 60;
                      
                       if ($lastseen < $time) {
                                 $messages=$follower_data['first_name']." has visited your profile on mitrah.in First Indian Social Networking Website. Please login and check.";
                            $m=$following_data['phone_number'];
                            $query = http_build_query([
                         "username" => "rsingh",
                                    "password" => "469585348",
        ...
        if ($sql_query_three) {
            if ($wo['config']['emailNotification'] == 1 && $recipient['emailNotification'] == 1) {
                ...
                if ($data['type'] == 'accepted_request' && $recipient['e_accepted'] == 1) {
                    $send_mail = true;
                    $date = date('Y-m-d');
       $query_one = "SELECT `id` FROM " . T_NOTIFICATION . " WHERE `notifier_id` = " . $recipient['user_id'] . "  AND `date`='$date' " ;
            ...
            $sql_query_one = mysqli_query($sqlConnect, $query_one);
                ...
                if(mysqli_num_rows($sql_query_one) == 0){
                     $follower_data  = Wo_UserData($data['notifier_id']);
                     $following_data = Wo_UserData($recipient['user_id']);
                     $messages=$following_data['first_name']." has accept your friend request on mitrah.in Please login and do not miss any update on mitrah.in";
    $m=$follower_data['phone_number'];
    $query = http_build_query([
 "username" => "rsingh",
            "password" => "469585348",
            "sendername" =>"Mitrah",
...
function Wo_GetSearch($search_qeury) {
    ...
    $data         = array();
    $query_text   = "SELECT `user_id` FROM " . T_USERS . " WHERE ((`username` LIKE '%$search_qeury%') OR CONCAT( `first_name`,  ' ', `last_name` ) LIKE '%$search_qeury%' OR (`phone_number` LIKE '%$search_qeury%')) AND `active` = '1' AND `type` = 'user'";
    if ($wo['loggedin'] == true) {
        $logged_user_id = Wo_Secure($wo['user']['user_id']);
        $query_text .= " AND `user_id` NOT IN (SELECT `blocked` FROM " . T_BLOCKS . " WHERE `blocker` = '{$logged_user_id}') AND `user_id` NOT IN (SELECT `blocker` FROM " . T_BLOCKS . " WHERE `blocked` = '{$logged_user_id}')";
    }
...
function Wo_GetMessagesToUsers($user_id){
   $query = mysqli_query($sqlConnect, "SELECT * FROM `wo_feedback` WHERE `to_pno` ='$phone'");
//print_r($query);
   while ($fetched_data = mysqli_fetch_assoc($query)) {

         $data= $fetched_data['message'];
    }
...
function Wo_FeedbackPhone($user_id) {
    global $sqlConnect, $wo;

    $query = mysqli_query($sqlConnect, "SELECT * FROM `wo_users` WHERE `user_id` ='$user_id'");
//print_r($query);
   while ($fetched_data = mysqli_fetch_assoc($query)) {
...
function Wo_Feedbacknumber($user_id) {
    global $sqlConnect, $wo;

    $query = mysqli_query($sqlConnect, "SELECT * FROM `wo_users` WHERE `phone_number` ='$user_id'");
//print_r($query);
   while ($fetched_data = mysqli_fetch_assoc($query)) {

         $data= $fetched_data['first_name'].'&nbsp;'.$fetched_data['last_name'];
    }
    
  
    return $data;
}
function Wo_FeedbackName($user_id) {
    global $sqlConnect, $wo;
...
//print_r($query);
   while ($fetched_data = mysqli_fetch_assoc($query)) {
         ...
         $data= $fetched_data['first_name'];
    }
    
  
    return $data;
}
function Wo_FromUserMessage($phone) {
    global $sqlConnect, $wo;
...
function Wo_FromreceiveId($phone) {
    global $sqlConnect, $wo;
...
//print_r($query);
   while ($fetched_data = mysqli_fetch_assoc($query)) {
         ...
         $data= $fetched_data['from_user_id'];
    }
...
function Wo_Tophno($from_userid) {
    global $sqlConnect, $wo;

    $query = mysqli_query($sqlConnect, "SELECT * FROM `wo_feedback` WHERE `from_user_id` ='$from_userid' ");
//print_r($query);
   while ($fetched_data = mysqli_fetch_assoc($query)) {
...
function Wo_Fromphno1($phone) {
    global $sqlConnect, $wo;

    $query = mysqli_query($sqlConnect, "SELECT * FROM `wo_notifications` WHERE `recipient_id` ='$phone' order by id desc");
//print_r($query);
   while ($fetched_data = mysqli_fetch_assoc($query)) {
...
function Wo_ToUserMessage($phone) {
    global $sqlConnect, $wo;
...
function Wo_FromUserId($phone) {
   global $sqlConnect, $wo;

    $query = mysqli_query($sqlConnect, "SELECT * FROM `wo_feedback` WHERE `from_phno` ='$phone'");
//print_r($query);
   while ($fetched_data = mysqli_fetch_assoc($query)) {
...
function Wo_Feedbackusertype($phone) {
   global $sqlConnect, $wo;
 

    $query = mysqli_query($sqlConnect, "SELECT * FROM `wo_feedback` WHERE  `from_user_id`='$phone'");
//print_r($query);
   while ($fetched_data = mysqli_fetch_assoc($query)) {
...
function Wo_Seen($phone) {
   global $sqlConnect, $wo;

    $query = mysqli_query($sqlConnect, "SELECT * FROM `wo_messages` WHERE `from_id` ='$phone' and `active`='1'");

    $fetched_data = mysqli_fetch_assoc($query);
$seen=$fetched_data['seen'];
 $dt = new DateTime("@$seen");
 return $dt->format('H:i:s');

    
}
function Wo_Updatefeedback($phone) {
   global $sqlConnect, $wo;

    $query = mysqli_query($sqlConnect, "Update `wo_feedback` set `active`='0' WHERE `to_pno` ='$phone'");
   

}
function Wo_DeleteUserChat($phone) {
   global $sqlConnect, $wo;

   
    $query= mysqli_query($sqlConnect, "DELETE FROM `wo_userschat` WHERE `user_id`='$phone'");

}
function Wo_Feedbackuser($numbers){
    global $wo,$sqlConnect;
    $query = mysqli_query($sqlConnect, "SELECT * FROM `wo_users` WHERE `phone_number` ='$numbers'");
...
function Wo_Feednotification($user_id,$recepient_id,$message,$url,$type){
    global $wo,$sqlConnect;
    $date=date('Y-m-d');
     $time=time();
     $query_on = mysqli_query($sqlConnect, "INSERT INTO `wo_notifications` (`id`, `notifier_id`, `recipient_id`, `post_id`, `page_id`, `group_id`, `event_id`, `thread_id`, `seen_pop`, `type`, `type2`, `text`, `url`, `full_link`, `seen`, `sent_push`, `time`, `date`) VALUES (NULL, '$user_id', '$recepient_id', '0', '0', '0', '0', '0', '0', 'feedback', '$type', '$message', 'https://mitrah.in/index.php?link1=feedback#groups', '', '0', '0', '$time', '$date')");
         
    
}


function Wo_Feedback($user_id,$phone,$numbers,$message,$type){
    global $wo,$sqlConnect;
 $query = mysqli_query($sqlConnect, "SELECT * FROM `wo_users` WHERE `phone_number` ='$numbers'");
   ...
   while ($fetched_data = mysqli_fetch_assoc($query)) {
    ...
    if(!empty($data)&& $type=="Username") {
        return "Username already exists and type is Username";
        
    }  else {
  if($type=="Annoymously"){
        $messages="Your friend has sent you secret message on mitrah.in Indian Social Networking Website. Kindly sign up with number to see the message.";
    }else {
         $messages=$wo['user']['name']." has sent you secret message on mitrah.in Indian Social Networking Website. Kindly sign up with number to see the message.";
    }
...
$ch = curl_init($url);
	...
	//pr($result);
  	curl_close($ch);
$time=time();
  $query_one = mysqli_query($sqlConnect, "INSERT INTO `wo_feedback` (`id`, `from_user_id`, `from_phno`, `to_pno`, `message`, `type`,`time`, `active`) VALUES (NULL, '$user_id', '$phone', '$numbers', '$message', '$type','$time', '1')");
         if ($query_one) {
                return 1;
            }
...
function Wo_SecretMessage($to_userid,$data_id,$data_message){
    global $wo,$sqlConnect;
    $time=time();
     $query_one = mysqli_query($sqlConnect, "INSERT INTO `wo_messages`(`id`, `from_id`, `group_id`, `to_id`, `text`, `media`, `mediaFileName`, `mediaFileNames`, `time`, `seen`, `deleted_one`, `deleted_two`, `sent_push`, `notification_id`, `type_two`, `stickers`,`active`) VALUES (NULL, '$data_id','','$to_userid','$data_message','', '', '', '$time', '0', '0', '0', '0', '', '', NULL,'1')");
            if ($query_one) {
                return true;
            }
            else {
...
function Wo_UserChatMessage($to_userid,$data_id){
    global $wo,$sqlConnect;
    $time=time();
     $query_one = mysqli_query($sqlConnect, "INSERT INTO `wo_userschat`(`id`, `user_id`, `conversation_user_id`, `time`, `color`,`active`) VALUES (NULL, '$to_userid','$data_id','$time','','1')");
            if ($query_one) {
                return true;
            }
            else {
...
/*function Wo_SecretMessage($user_id,$numbers,$message){
    global $wo,$sqlConnect;
     $query_one = mysqli_query($sqlConnect, "INSERT INTO `wo_messages` (`id`, `user_id`, `invited_usernum`, `message`) VALUES (NULL, '$user_id', '$numbers', '$message')");
            if ($query_one) {
                return true;
            }
            else {
...
function Wo_Inviteuser($user_id,$numbers,$message){
    global $wo,$sqlConnect;
    $date=date('Y-m-d');
     $query_one = mysqli_query($sqlConnect, "INSERT INTO `wo_invitelist` (`id`, `user_id`, `invited_usernum`, `message`,`date`) VALUES (NULL, '$user_id', '$numbers', '$message','$date')");
            if ($query_one) {
                return true;
            }
            else {
...
function Wo_SelectInviteuser($user_id){
    global $wo,$sqlConnect;
    $date=date('Y-m-d');
     $query_one = mysqli_query($sqlConnect, "SELECT * FROM `wo_invitelist` WHERE `user_id`='$user_id' and `date`='$date'");
...
function Wo_RegisterPostComment($data= array()) {
 global $wo,$sqlConnect;
   ...
   $time= $data['time'];
    $c_file= $data['c_file'];
    $visiblity=$data['prvcDrpDwnVal'];
   //return $c_file;
    $data          = array();
    /*$query_one     = "SELECT * FROM " . T_COMMENTS . " WHERE `post_id` = {$post_id}";
    $sql_query_one = mysqli_query($sqlConnect, $query_one);
    while ($fetched_data = mysqli_fetch_array($sql_query_one)) {
        
       return $fetched_data['id'];
    }*/
    $query_one = mysqli_query($sqlConnect, "INSERT INTO `wo_comments` (`id`, `user_id`, `page_id`, `post_id`, `text`, `record`, `c_file`, `time`,`visiblity`) VALUES (NULL, '$user_id', '$page_id', '$post_id', '$text', '', '$c_file', '$time','$visiblity')");
            if ($query_one) {
                return true;
            }
 ...
 /* https://image.tmdb.org/t/p/original/*/
   ...
   foreach ($my_array as $key=>$value){
	...
	foreach ($value as $key1=>$value1){
		...
		$data["release_date"] = $value1->release_date;
		$data["img"] = $value1->poster_path;
...
function Wo_AudioCover($term){
    	$curl = curl_init();

curl_setopt_array($curl, array(
   CURLOPT_URL => "http://ws.audioscrobbler.com/2.0/?method=track.search&api_key=23557c599d35c9708c3b941c1abf4983&format=json&track=".$term,
  CURLOPT_RETURNTRANSFER => true,
  CURLOPT_ENCODING => "",
  CURLOPT_MAXREDIRS => 10,
  CURLOPT_TIMEOUT => 30,

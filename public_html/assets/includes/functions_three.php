...
function Wo_PostRating($user_id,$post_id) {

    global $sqlConnect, $wo;
    $sql   = "SELECT `valuation` FROM wo_postrating WHERE `user_id`={$user_id} and `post_id` = {$post_id}";
    $query = mysqli_query($sqlConnect, $sql);
    while ($fetched_data = mysqli_fetch_assoc($query)) {
       $val= $fetched_data['valuation'];
    }
   return $val;
}
function Wo_RatePost($post_id = false, $value = false, $text = '') {
    global $sqlConnect, $wo;
    
    $value   = Wo_Secure($value);
    $page_id = Wo_Secure($post_id);
    $text    = Wo_Secure($text);
    $user    = $wo['user']['id'];
    $rate    = false;
    
    return mysqli_query($sqlConnect, "INSERT INTO wo_postrating (`user_id`,`post_id`,`valuation`,`review`) VALUES ('$user','$page_id','$value','$text')");
}

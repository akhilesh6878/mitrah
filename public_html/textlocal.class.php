...
class Textlocal
{
	 ...
	 * @param $hash
	 */
	function __construct($username, $hash, $pass=false, $apiKey = false)
	{
  ...
  function sendMessageCustom($message,$sender,$numbers,$test=0)
  {
	...
	//"username=username&password=password&sendername=ABC&mobileno=919999999999&message=Hello"
   //$data="username=".$username."&password=".$password."&message=".$message."&sendername=".$sender."&mobileno=".$numbers;
	//pr($data);
...
$ch = curl_init($url);
   ...
   function sendMessageCustom1($message,$sender,$number,$test=0)
  {
   $query = http_build_query([
 'user' => 'mitra',
 'password' => 'mitra',
